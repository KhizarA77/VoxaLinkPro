...
exports.connectWallet = async (req, res) => {
    ...
    try {
        ...
        // Create JWT
        const accessToken = jwt.sign({ 'walletAddress':WA }, process.env.JWT_SECRET, { expiresIn: '5m' });
        console.log(`JWT Access Token Created Successfully ${accessToken}`);

        const refreshToken = jwt.sign({ 'walletAddress':WA }, process.env.JWT_REFRESH_SECRET, { expiresIn: '5h' });
        await pool.query(`UPDATE WALLETS SET refresh_token = $1 WHERE wallet_address = $2`, [refreshToken, WA]);
