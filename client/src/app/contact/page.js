...
const ContactPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateEmail(email)) {
      ...
      try {
        const response = await axios.post('https://voxalink-discord-express-api-1840f6529c67.herokuapp.com/support', {
          email: email,
          message: message
        });
        ...
        if (response.status === 200) {
          ...
          setResponseMessage("Too many requests at this time. Please try again later.");
        }
      } catch (error) {
    ...
    <div className="flex flex-col min-h-screen bg-[#17181d] text-black justify-center items-center px-4">
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <p className="text-sm text-gray-400 mb-6 text-center">
          If you're seeking information about our AI-driven solutions, require help with our offerings, or are interested in exploring partnership opportunities, our dedicated team is ready to offer quick and tailored assistance.
        </p>
        ...
        <button
          ...
          className={`relative w-full py-2 px-4 rounded-full shadow-md focus:outline-none text-lg transition duration-500 ease-in-out ${
            isSent ? 'bg-purple-500' : 'bg-blue-600 hover:bg-blue-700 hover:-translate-y-1 hover:scale-105'
          }`}
          style={{
            border: '1px solid transparent',
            backgroundImage: isSent ? 'none' : 'linear-gradient(to right, #9d50bb, #ff6db3)', // Gradient from purple to pink
            opacity: isSent ? '0.5' : '1',
            boxShadow: 'inset 0 1px 1px rgba(255, 255, 255, 0.3), 0 1px 2px rgba(0, 0, 0, 0.1)'
          }}
        >
          {isSent ? 'Sent' : 'Send'}
          {!isSent && (
        ...
        {responseMessage && <p className="text-center text-red-700">{responseMessage}</p>}
      </div>
