...
import { abi } from "../../constants/abi"; // Replace with your contract ABI
...
export default function PreSaleCard() {
  ...
  const phaseStartDates = ["2023-11-01", "2024-01-01", "2024-03-01"]; // Array of phase start dates
  const phaseEndDates = ["2023-11-30", "2024-02-28", "2024-04-30"]; // Array of phase end dates
  ...
  const [currentTokenPriceUSD, setCurrentTokenPriceUSD] = useState(0.042); // Default token price in USD
  const [usdToPay, setUsdToPay] = useState("");
  const [tokensToReceive, setTokensToReceive] = useState("");
  const ETHPriceUSD = 1500; // Constant ETH price in USD for calculation

  const calculateTokensFromUSD = (usd) => {
    const tokens = usd / currentTokenPriceUSD;
    setTokensToReceive(tokens.toFixed(2)); // Keeping two decimal points for tokens
  };

  const calculateUSDFromTokens = (tokens) => {
    const usd = tokens * currentTokenPriceUSD;
    setUsdToPay(usd.toFixed(2)); // Keeping two decimal points for USD
  };

  const handleUSDChange = (event) => {
    setUsdToPay(event.target.value);
    calculateTokensFromUSD(event.target.value);
  };

  const handleTokensChange = (event) => {
    setTokensToReceive(event.target.value);
    calculateUSDFromTokens(event.target.value);
  };
  ...
  return (
    <div className="bg-[#6664643f] p-8 rounded-xl backdrop-blur-lg shadow-xl w-full lg:w-[35rem] mx-auto mt-10 text-center">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        ...
        <button
          ...
          className={`p-2 rounded ${
            selectedOption === "ETH"
              ? "bg-[#7A77F3] text-white border-none"
              : "bg-transparent text-white border border-white"
          } hover:${selectedOption !== "ETH" ? "bg-[#7A77F3]" : ""}`}
        >
          ETH
        </button>
        ...
        <button
          ...
          className={`w-full p-2 rounded ${
            selectedOption === "CARD"
              ? "bg-[#26A17B] text-white border-none"
              : "bg-transparent text-white border border-white"
          } hover:${selectedOption !== "CARD" ? "bg-[#26A17B]" : ""}`}
        >
          CARD
        </button>
      </div>
      <div className="mb-4 text-white">
        ...
        <input
          className="bg-white text-slate-800 pl-2 py-1 md:p-2 rounded w-full mt-2"
          type="number"
          value={usdToPay}
      ...
      <button className="bg-[#ffb341] hover:bg-[#dfb563] w-full px-4 py-2 rounded shadow-md mb-2">
        Buy Token
      </button>
    </div>
  );
